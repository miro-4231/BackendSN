...
async def root(
    ...
    statement = select(model.Posts).offset(offset).limit(limit)
...
async def get_user_posts(user_id: int, current_user: Annotated[schemas.User_out, Depends(oauth2.get_current_user)], session: Annotated[Session, Depends(utils.get_db)],
                        limit: int = Query(default=10, gt=0, le=100, description="Maximum number of items to return"),
                        offset: int = Query(default=0, ge=0, description="Number of items to skip")):
...
async def case_vote(post_id: int, vote_in: schemas.VoteCreate,
    ...
    if not vote_target:
        new_vote = model.Votes(user_id=current_user.id, post_id=post_id, direction=vote_in.direction)
        session.add(new_vote)
        user = session.get(model.Users, current_user.id)
        if vote_in.is_super :
            user.super_vote_balance -= 1
        if vote_in.direction == 1:
            post_target.votes += 1 * SUPER_VOTE_MULTIPLIER if vote_in.is_super else 1
        elif vote_in.direction == -1:
            post_target.votes -= 1 * SUPER_VOTE_MULTIPLIER if vote_in.is_super else 1
        else: 
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Direction must be 1 or -1")
    elif vote_target:
...
async def delete_vote(post_id: int,
    ...
    if vote_target:
        ...
        if vote_target.is_super:
            user.super_vote_balance += 1
        if vote_target.direction == 1:
            post_target.votes -= 1 * SUPER_VOTE_MULTIPLIER if vote_target.is_super else 1
        elif vote_target.direction == -1:
            post_target.votes += 1 * SUPER_VOTE_MULTIPLIER if vote_target.is_super else 1
        else: 
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Direction must be 1 or -1")
